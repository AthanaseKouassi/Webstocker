...
import com.webstocker.domain.Authority;
import com.webstocker.domain.User;
import com.webstocker.repository.AuthorityRepository;
import com.webstocker.repository.UserRepository;
import com.webstocker.repository.search.UserSearchRepository;
import com.webstocker.security.AuthoritiesConstants;
import com.webstocker.service.MailService;
import com.webstocker.service.UserService;
import com.webstocker.web.rest.dto.ManagedUserDTO;
import com.webstocker.web.rest.dto.UserDTO;
...
public class UserResource {
    ...
    @RequestMapping(value = "/users/aimas",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
//    @Transactional(readOnly = true)
    public List<User> findAllUserAimas() {
        return userRepository.findAllUserAimas();
    }
    ...
    @Timed
    @Transactional(readOnly = true)
    public ResponseEntity<List<ManagedUserDTO>> toutLesUtilisateur()
            throws URISyntaxException {
        List<User> liste = userRepository.findAll();
//        List<ManagedUserDTO> managedUserDTOs = page.getContent().stream()
        List<ManagedUserDTO> managedUserDTOs = liste.stream()
                .map(ManagedUserDTO::new)
                .collect(Collectors.toList());
//        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/users");
//        return new ResponseEntity<>(managedUserDTOs, headers, HttpStatus.OK);
    ...
    @RequestMapping(value = "/users/allusers-by-page",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @Transactional(readOnly = true)
    public ResponseEntity<List<ManagedUserDTO>> toutLesUtilisateurPage( @RequestParam(name = "page", defaultValue = "0") int page,
            @RequestParam(name = "size", defaultValue = "3") int size) throws URISyntaxException {
        Page<User> listePage = userRepository.findAll(new PageRequest(page, size));
        List<ManagedUserDTO> managedUserDTOs = listePage.getContent().stream()
                .map(ManagedUserDTO::new)
                .collect(Collectors.toList());
        return new ResponseEntity<>(managedUserDTOs, HttpStatus.OK);
    }
