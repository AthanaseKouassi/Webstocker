 ...
 * @author Athanase
 */
public class BonDeSortieTransfertReport {
    private AggregationSubtotalBuilder<Long> totalQuantiteProduit;
    List<BonDeSortieWrapper> reportData;

    public BonDeSortieTransfertReport(List<LigneBonDeSortie> reportData) {
...
//            report.addColumn(col.column("Designation", "nomProduit", type.stringType()));
//            report.addColumn(col.column("Numéro lot","numeroLot", type.stringType()));
//            report.addColumn(col.column("Quantite", "quantite", type.longType()));
    ...
    private ComponentBuilder<?, ?> bordereauInfoClientDemandeurComponent(List<BonDeSortieWrapper> bonDeSortieWrapper) {
        ...
        switch(typesortie){
            case "VENTE":
                destinataire =  cmp.horizontalList().add(cmp.text("Client : " + "client")).setBackgroundComponent(rect).setFixedWidth(200); 
            break;
            case "TRANSFERT":
            ...
            break;
            case "PROMOTION":
               destinataire =  cmp.horizontalList().add(cmp.text("Beneficiaire : " + "benficiaire")).setBackgroundComponent(rect).setFixedWidth(200);     
             break;
            default:
                destinataire =  cmp.horizontalList().add(cmp.text("Client : " + "client")).setBackgroundComponent(rect).setFixedWidth(200);
             break;        
